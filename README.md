# Hi there, my name is Max! 👋

An IT student currently undergoing professional retraining (*Umschulung*) at **IBB Magdeburg**, preparing for the official **IHK certification exam** as an **IT Specialist for Application Development** (*Fachinformatiker für Anwendungsentwicklung*). 

I am focused on mastering full-stack software development step-by-step through hands-on practice.

---

### 🚀 About Me

- 💼 **Background:** 8 years of professional experience in international logistics and sales management, transitioning into the IT sector.
- 🎓 **School Curriculum:** Covering core IT fundamentals including database design, networking (*ITTS / Netzwerktechnik*), *Wirtschaftsprozesse*, and project quality management (*PQSM*).
- 🌍 **Languages:** Fluent in German, English, Russian, and Latvian.

---

### 💻 Tech Stack & Learning Focus

#### **Frontend**
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
- Building solid foundations in responsive layouts and core JS scripting.

#### **Backend & Databases**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white) ![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=mariadb&logoColor=white)
- Developing application logic with Python, managing relational databases with SQL, and building web backends with Flask.

#### **Tools & Workflow**
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) ![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white) ![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)

---

### 🛠️ Current Projects & Experiments
- 📚 **Meine Bibliothek:** Eine minimalistische Webanwendung zur Verwaltung einer persönlichen Büchersammlung, entwickelt im Rahmen der FIAE-Ausbildung.
  [👉 Live Demo hier ansehen](https://meine-bibliothek.onrender.com)
- 🟢 **Matrix Portfolio (In Progress):** A personal frontend practice project using HTML, CSS, and interactive JavaScript with a Matrix-themed aesthetic.
- 🤖 **Agentic AI Exploration:** Personally experimenting with workflows, LangChain, and AI tools to explore the concept of autonomous computer operators and automated assistants.
- 👥 **Team School Projects (In Progress):** Educational projects built within the curriculum to prepare for the certification exam, while practicing working in a team.
